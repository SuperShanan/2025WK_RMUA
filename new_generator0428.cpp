...
std::vector<Eigen::Vector3d> list_original{{3.1,0.57,1.20},{3.1,0,1.20},{3.1,-4.0,1.2}};//所有需要停的位置以及预先点
std::vector<Eigen::Vector3d> over_circle;
std_msgs::Int32 msg;

// geometry_msgs::PoseStamped previous_position;
int stop[5] = {1,2};    //需要重归划的点的下标，会停下的点下标
...
void rviz_goal_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)  //这里没有用到但要留着
{
    goal_pt = msg->pose;
...
// void getthroughCallback(const std_msgs::Int32::ConstPtr &msg) 
// {
//   get_through=msg->data;
//   //接收当前端点的下标
// }
...
void eight_goal_cb(const geometry_msgs::PoseStamped target_info)    //第一次规划到stop[0]
{
    ...
    {
        geometry_msgs::Pose tmp_pt = goal_pt;
...
{   
    //if 偏差太大，用粗略点走
    goal_pt.position = odom.pose.pose.position;
    goal_list.poses.push_back(goal_pt);     //1.插入odom
...
void circleposeCallback(const geometry_msgs::Point::ConstPtr &shijuepose)
{
    ...
    houbanduan(stop[duandian_index]+1);   //当前stop下标的下一个下标
    duandian_index++; 
}
...
int main(int argc, char **argv)
{
    ...
    // timergetend_ = nh.createTimer(ros::Duration(0.5), checkgetend);  //0.02s检查一下是否到达目标点，到达后更新目标点位
